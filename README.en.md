...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 002 retro hand-drawn editorial illustration below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 002 retro hand-drawn editorial illustration right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `002.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different narrative contours, selective enlargements, loose supports, and type relationships.
...
The image model creates narrative silhouette, hesitant contour, selective feature enlargement, loose wash, analogous colour, old-paper mixed media, source-specific metaphor, and misregistered type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
