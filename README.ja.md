<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 002 プロジェクトバナー" width="1200">
</p>

<div align="center">

# 🦁 XXD Panel 002｜遊び心のバウハウス

### 写真を、物語る輪郭、ためらう手線、版ずれ文字を持つレトロ編集画へ
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 002 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 002 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 002 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 002 作例 8"></td>
  </tr>
</table>

## 追加 3:4 上下構成作例

上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 002 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 002 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 002 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 002 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 002 追加上下作例 4"></td>
  </tr>
</table>
...
> 物語る輪郭 · ためらう線 · 類似色 · 部分拡大 · 版ずれ文字

XXD Panel 002 は、Codex と互換 Agent のための画像生成 Skill です。写真の主体、見える感情、姿勢、空間関係を正確に残し、ためらう輪郭、物語を担う一つの部分拡大、淡いにじみ、古い印刷の版ずれによって、保存された昔のファッション絵本と現代美術誌の間にある一頁へ翻訳します。
...
**Panel 002** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真を、物語る輪郭、ためらう手線、版ずれ文字を持つレトロ編集画へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 002 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 002 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、物語る輪郭、ためらう手線、版ずれ文字を持つレトロ編集画へ |
| **ひと目で分かる特徴** | 物語る輪郭 · ためらう線 · 類似色 · 部分拡大 · 版ずれ文字 |
...
```text
元写真の事実を固定 → 写真固有の視覚的比喩を見つける → 人の手の揺れを残した細線で簡略化 → 水彩・ガッシュ・色鉛筆・パステルを重ねる → 温かな紙の余白を守る → 控えめなレトロ手書き文字を加える
```

無関係な写真に替えても主体、比喩、身振り、配色、文字がほぼ成立するなら、それは 002 ではありません。

## 完成作品で見分けやすいスタイル特性

- **識別できる物語的輪郭：** 元写真固有の手掛かり、姿勢、空間関係を残し、ためらう線でも主体を失いません。
- **写真固有の変換：** 動き、構造、環境、感情から、自然で気の利いた一つの視覚的比喩を導き、固定の仕掛けを当てません。
- **選択的な部分拡大：** 身元、見える感情、物語を担う一箇所だけを拡大し、任意に変形しません。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 002 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-002 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-002 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-002 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-002 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 002 を適用し、共通設定は一度だけ解決
/xxd-panel-002 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-002` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-002-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-002-prompt.en.md)
